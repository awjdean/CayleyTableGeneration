    ...
    def generateCayleyTable(self, **parameters):
        ...
        for table_index_row in range(len(self.cayley_table_states.index)):
            for table_index_column in range(len(self.cayley_table_states.columns)):
                ...
                # ------------------------------------------------------------------------------------------------------
                # TODO: find_element_equivalents_in_state_cayley_table function
                #   --> CHECKED: same as find_element_equivalents_in_state_cayley_table function.
        ...
        while len(cayley_table_candidate_elements) > 0:
            ...
            # ----------------------------------------------------------------------------------------------------------
            # TODO: find_element_equivalents_in_state_cayley_table function.
            #   ---> Slightly different (.index <--> .columns), but should have no material impact.
            ...
            for temp_ec_label in temp_ecs.keys():
                ...
                # ------------------------------------------------------------------------------------------------------
                # TODO: add_element_to_state_cayley_table function
            ...
            # ----------------------------------------------------------------------------------------------------------
            # TODO: add_element_to_state_cayley_table
            ...
            # Add row to state Cayley table.
            candidate_state_cayley_table_row = pd.Series(candidate_state_cayley_table_row,
                                                         name=candidate_element)
            candidate_state_cayley_table_row = pd.DataFrame([candidate_state_cayley_table_row],
                                                            columns=self.cayley_table_states.columns)
            self.cayley_table_states = pd.concat([self.cayley_table_states,
                                                  candidate_state_cayley_table_row])
