...
from Environments.gridworld import BaseGridworld, draw_base_gridworld2d, Action2D
...
class Gridworld2DBlock(BaseGridworld):
    ...
    def __init__(self, grid_size, initial_agent_position, initial_block_position, **kwargs):
        ...
        self.transition_matrix = generate_transition_matrix(all_states=self._possible_states,
...
def generate_transition_matrix(all_states, grid_size, minimum_actions):
    transition_matrix = {}
    for state, action in itertools.product(all_states, minimum_actions):
        block_position = state[2:4]
        agent_position = state[0:2]

        new_agent_position = Action2D(action).apply(position=agent_position, grid_size=grid_size)
        ...
        if new_agent_position == block_position:
            new_block_position = Action2D(action).apply(position=block_position, grid_size=grid_size)
        else:
...
def check_variables(grid_size, block_position, **kwargs):
    # TODO: check block position not in same position as agent.
